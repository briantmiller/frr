...
"""
test_nhrp_cisco_group.py: Test NHS Cisco vendor group attribute
"""
...
TOPOLOGY = """
                              ...
                              --------+------------------+--------
                                                         |
                                                         |.6
         GRE P2MP between NHS and all NHC          +-----+------+
               172.16.1.x/32                       |            |
                                                   |            |
                                                   |   Router   |
                                                   |            |
...
def build_topo(tgen):
    ...
    # Create 7 routers
    for rname in ["nhs1", "nhc1", "nhc2", "router", "host"]:
...
def setup_module(mod):
    logger.info("NHRP Cisco group attribute:\n {}".format(TOPOLOGY))
...
def test_nhrp_cisco_group_spoke_to_hub():
    """
    Assert that NHRP spoke to hub with Cisco group is being communicated
    """
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)

    logger.info("Checking NHRP cache for convergence, including Cisco group")
    router_list = tgen.routers()
...
def test_nhrp_cisco_group_spoke_to_spoke():
    """
    Assert that if shortcut created then cisco group is also communicated
    """
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    logger.info("Testing NHRP shortcuts with Cisco group")
    ...
    #Check group on NHS
    logger.info("Checking Cisco group spoke to spoke")
    router_list = tgen.routers()
    for rname, router in router_list.items():
        if "nhc" not in rname:
            continue

        json_file = "{}/{}/nhrp_shortcut.json".format(CWD, router.name)
        expected = json.loads(open(json_file).read())
        test_func = partial(
            topotest.router_json_cmp, router, "show ip nhrp json", expected
        )
        _, result = topotest.run_and_expect(test_func, None, count=40, wait=0.5)

        output = router.vtysh_cmd("show ip nhrp json")
        logger.info(output)
